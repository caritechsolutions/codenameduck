...
async function startStack() {
  ...
  migrate(db);
  const logs = [];
  const srv = createServer({ db, tenantsDir, adminDist: null, pollIntervalS: 15, log: (m) => logs.push(m) });
  ...
  const api = (method, p, body, cookie) => new Promise((resolve, reject) => {
    ...
    const req = http.request({ host: '127.0.0.1', port, path: p, method, headers: { 'Content-Type': 'application/json', ...(cookie ? { Cookie: cookie } : {}), ...(data ? { 'Content-Length': Buffer.byteLength(data) } : {}) } }, (res) => {
      let t = ''; res.on('data', (c) => { t += c; }); res.on('end', () => resolve({ status: res.statusCode, json: t ? JSON.parse(t) : null, cookie: (res.headers['set-cookie'] || [])[0]?.split(';')[0] }));
    });
  ...
  return { ...srv, db, port, base, api, login, logs, close, url: base + '/procentric/application/index.html' };
}
