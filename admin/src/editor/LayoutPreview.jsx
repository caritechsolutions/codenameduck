...
export default function LayoutPreview({ doc, screenId, width = 960, tenant, openPage = null }) {
  const canvas = doc.canvas || { w: 1920, h: 1080 };
  const scale = width / canvas.w;
  const stageRef = useRef(null);
  const [now, setNow] = useState(() => new Date());
  useEffect(() => { const t = setInterval(() => setNow(new Date()), 1000); return () => clearInterval(t); }, []);
  const firstMenu = draw.visibleZones(doc, screenId, openPage).find((z) => z.type === 'menu' || z.type === 'app_launcher');
  useEffect(() => {
  ...
  }, [doc, screenId, openPage, tenant, firstMenu && firstMenu.id]); // eslint-disable-line react-hooks/exhaustive-deps
