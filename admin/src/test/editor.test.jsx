...
function Harness({ initial = DOC }) {
  ...
  return <ToastProvider>
    <EditorToolbar doc={doc} selectedIds={sel} onChange={setDoc} onSelect={setSel} screenId={screenId} canUndo={false} canRedo={false} onUndo={() => {}} onRedo={() => {}} />
    <CanvasEditor doc={doc} onChange={setDoc} selectedIds={sel} onSelect={setSel} screenId={screenId} width={960} />
    <ZonePanel doc={doc} selectedIds={sel} onChange={setDoc} onSelect={setSel} screenId={screenId} setScreenId={setScreenId} />
...
describe('canvas editor', () => {
  ...
  it('video resize keeps 16:9; screen toggle; toolbar delete; canvas panel edits keys', () => {
    render(<Harness />);
    const tv = document.querySelector('[data-zone="tv"]');
    down(tv, 0, 0); up(0, 0);
  ...
  it('multi-select with shift, drag moves both, align + lock from the toolbar', () => {
    ...
    down(w, 100, 100);
    fireEvent.pointerMove(window, { clientX: 100, clientY: 150 });   // +0,+100 canvas px (Alt free? no: snapped 160 → y 160; guides: welcome bottom 250? no)
    up(100, 150);
    let d = docOf();
    ...
    // locked zones do not move on arrow keys
    fireEvent.keyDown(screen.getByRole('application'), { key: 'ArrowRight', shiftKey: true });
